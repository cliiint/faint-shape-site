...
import { navLinks, navLinkItem, navLinkText, showLinkItem, nextShowContainer, showGhost, showUnderline } from './navigation.module.css'
    ...
    if (nextShow) {
      ...
      return (
        <div className={ nextShowContainer }>
          ...
          <Link className={ showLinkItem } to={`/shows/${nextShow.slug}`}>
            <span className={showGhost}>👻</span>
            { formattedDate + ' ' + nextShow.location }
            <span className={showGhost}>👻</span>
          </Link>
        </div>
      );
